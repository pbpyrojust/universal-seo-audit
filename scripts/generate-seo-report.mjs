...
for (const row of sections.slice(0, 10)) out.push(`- ${row.section}: ${row.page_count} page(s), avg issues ${row.avg_issue_count}, orphan candidates ${row.orphan_candidates}`);
out.push("");
out.push("## Orphan candidates");
for (const row of crawl.filter((r)=>r.orphan_candidate==="yes").slice(0, 15)) out.push(`- ${row.page_url} | section ${row.section} | depth ${row.internal_link_depth || "n/a"} | inlinks ${row.inlinks}`);
out.push("");
