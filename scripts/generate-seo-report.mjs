...
out.push("");
out.push("## What this audit does");
out.push("Phase 3 extends the earlier SEO audit phases with stronger technical SEO signals and lightweight render diagnostics for development, staging, protected, and noindex environments. It is intended to catch problems before launch or before a public crawl is possible.");
out.push("");
out.push("## Phase 3 additions");
out.push(`- Missing html lang issues: ${count("missing_lang")}`);
out.push(`- Missing viewport issues: ${count("missing_viewport")}`);
out.push(`- Invalid JSON-LD issues: ${count("invalid_jsonld")}`);
out.push(`- X-Robots-Tag noindex issues: ${count("noindex_header")}`);
out.push(`- Invalid hreflang issues: ${count("hreflang_invalid")}`);
out.push(`- Heavy DOM flags: ${count("heavy_dom")}`);
out.push(`- High resource count flags: ${count("high_resource_count")}`);
out.push("");
...
out.push("");
out.push("## Phase 2 findings still tracked");
out.push(`- Duplicate meta descriptions: ${count("duplicate_meta_description")}`);
...
out.push("2. Fix missing/duplicate titles, canonical issues, and missing H1s");
out.push("3. Fix missing viewport/lang issues, invalid structured data, and noindex directives that should not exist");
out.push("4. Review duplicate meta descriptions, thin-content pages, heavy-page signals, and image alt problems");
out.push("");
